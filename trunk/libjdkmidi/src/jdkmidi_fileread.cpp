...
//
  ...
  void MIDIFileRead::ReadTrack()	
  {
    ...
    while( to_be_read > 0 && !abort_parse )
    {
      ...
      {
        // Special thanks to "Peter Schwind" <pschwind@gmx.de> 
        // for the answer to the question and the fix for:
        // "is running status to be cleared or not when meta event happens?"
        // the answer is no.
